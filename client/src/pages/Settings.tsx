...
import { Lock, Bell, Moon, Key, LogOut, ChevronRight, User, Phone, Image as ImageIcon, Check, ShieldCheck, Database, Terminal, FileJson, Play, FlaskConical } from 'lucide-react';
import { useApp } from '../context';
import { encryptMessage, getDebugInfo } from '../utils/encryption';
import { runEncryptionTests } from '../utils/tests';

const SettingsPage: React.FC = () => {
  const { currentUser, updateUser, theme, toggleTheme, notificationsEnabled, toggleNotifications, logout, encryptionKey } = useApp();
  ...
  const [rawDbSnippet, setRawDbSnippet] = useState('');
      ...
      if (showDebugModal) {
          ...
          if (saved) {
             try {
                ...
                // Get a sample of the first message of first chat
                const sample = parsed[0]?.messages[parsed[0]?.messages.length - 1];
                setRawDbSnippet(JSON.stringify(sample, null, 2) || 'No messages');
             } catch(e) {
                setRawDbSnippet('Error parsing DB');
             }
          } else {
             setRawDbSnippet('Database empty');
          }
      }
  }, [showDebugModal]);
  ...
  const handleRunUnitTests = async () => {
      setUnitTestResult('Running tests...');
      const result = await runEncryptionTests();
    ...
    <div className="h-full bg-[var(--bg-main)] p-6 flex flex-col overflow-y-auto no-scrollbar pb-20 text-[var(--text-main)] relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-[var(--bg-secondary)] w-full max-w-sm p-8 rounded-[2.5rem] shadow-2xl border border-[var(--border-color)] flex flex-col items-center text-center transform transition-all scale-100 ring-1 ring-white/10">
              ...
              >
                 ...
                 <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition text-[10px] uppercase text-[var(--bg-accent)] font-bold">
                    Копировать
                 </div>
              </div>
              ...
              <button 
                ...
                className="w-full bg-green-600 hover:bg-green-500 text-white py-4 rounded-2xl font-bold uppercase tracking-widest shadow-lg hover:shadow-green-500/20 active:scale-95 transition-all text-sm"
              >
                Я защищен!
              </button>
           </div>
      ...
      {showDebugModal && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
           <div className="bg-[var(--bg-secondary)] w-full max-w-2xl h-[85vh] md:h-auto md:max-h-[85vh] p-6 rounded-t-[2.5rem] md:rounded-[2.5rem] shadow-2xl border border-[var(--border-color)] flex flex-col overflow-hidden ring-1 ring-white/10">
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-6 px-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="bg-[var(--bg-tertiary)] p-4 rounded-2xl">
                        <div className="text-[10px] uppercase text-[var(--text-secondary)] mb-1">Active Key ID</div>
                        <div className="font-mono text-sm font-bold text-green-500">{debugInfo?.currentKeyId || '...'}</div>
                     </div>
                     <div className="bg-[var(--bg-tertiary)] p-4 rounded-2xl">
                        <div className="text-[10px] uppercase text-[var(--text-secondary)] mb-1">Total Keys</div>
                        <div className="font-mono text-sm font-bold">{debugInfo?.totalKeys || 0}</div>
                     </div>
                  </div>
                  ...
                  <div className="bg-[var(--bg-main)] p-4 rounded-3xl border border-[var(--border-color)]">
                      <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-2">
                              <FlaskConical size={16} className="text-[var(--bg-accent)]" />
                              <span className="text-xs font-bold uppercase">Unit Tests</span>
                          </div>
                          <button onClick={handleRunUnitTests} className="bg-[var(--bg-accent)] hover:bg-opacity-80 text-white px-3 py-1 rounded-lg text-[10px] font-bold uppercase">
                              Запустить тесты
                          </button>
                      </div>
                      
                      {unitTestResult && (
                          <div className="bg-[#1a1a1a] p-4 rounded-xl font-mono text-[10px] text-green-300 overflow-x-auto whitespace-pre-wrap leading-relaxed">
                              {unitTestResult}
                          </div>
                      )}
                  </div>
                  ...
                  <div className="bg-[var(--bg-main)] p-4 rounded-3xl border border-[var(--border-color)]">
                      <div className="flex items-center gap-2 mb-4">
                          <Play size={16} className="text-[var(--bg-accent)]" />
                          <span className="text-xs font-bold uppercase">Live Encryption Test</span>
                      </div>
                      ...
                      <div className="flex gap-2 mb-4">
                          <input 
                            ...
                            className="flex-1 bg-[var(--bg-tertiary)] rounded-xl px-3 py-2 text-sm focus:outline-none"
                            placeholder="Type text..."
                          />
                          <button onClick={handleRunEncryptionTest} className="bg-[var(--bg-accent)] text-white px-4 rounded-xl text-xs font-bold uppercase">Run</button>
                      </div>

                      <div className="bg-[#1a1a1a] p-4 rounded-xl font-mono text-xs text-green-400 overflow-x-auto whitespace-pre-wrap">
                          {testOutput || '// Click Run to generate ciphertext'}
                      </div>
                  </div>
                  ...
                  <div className="bg-[var(--bg-main)] p-4 rounded-3xl border border-[var(--border-color)]">
                      <div className="flex items-center gap-2 mb-4">
                          <FileJson size={16} className="text-[var(--bg-accent)]" />
                          <span className="text-xs font-bold uppercase">Database Snapshot (Latest Msg)</span>
                      </div>
                      <div className="bg-[#1a1a1a] p-4 rounded-xl font-mono text-xs text-blue-300 overflow-x-auto whitespace-pre-wrap break-all">
                          {rawDbSnippet}
                      </div>
                      <div className="mt-2 text-[10px] text-[var(--text-secondary)] text-center uppercase">
                          Source: localStorage['openkey_chats']
                      </div>
                  </div>
